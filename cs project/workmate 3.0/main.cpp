...
void order()
{
	...
	cout << "\t U Can Order A Service By Filling The Form. Thank You! \n\n";

	cout << "Job Description: ";
	cin.ignore();
	getline(cin, Job.name);
	cout << "Skill: ";
	getline(cin, Job.skill);
	cout << "Quantity: ";
    cin >> Job.quant;
	cout << "Price Per Hour: $ ";
	cin >>Job.pph;
	...
	cin.ignore();
	getline(cin, Job.email);

	// Open file for writing
    ofstream outfile("order.txt", ios::app);

    // Write user data to file
    outfile << Job.name << " | " << Job.skill  << " | " << Job.quant << " | " << Job.pph <<"|"<< Job.email << " | " << time << endl;
...
void searchorder()
{
	...
	cout << "| Job name | skill | Quantity | Budget($) | Created on | \n\n";
	while (getline(file, line)) {
...
void dashboard()
{
	...
	cout << "|  job  |  skill  |  quantity  |  Budget($)  | Email |  Date  | \n";
	ifstream file("order.txt");
	if (file.is_open()) {
		string line;
		while (getline(file, line)) {
